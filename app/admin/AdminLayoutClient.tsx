...
export default function AdminLayoutClient({ 
  ...
  // If no user profile but not on login, something went wrong with the session server-side
  if (!userProfile) {
    return null;
  }

  const handleLogout = () => {
    // Clear cookies/localStorage just in case we have stale data
  ...
  const filteredSidebar = SIDEBAR_ITEMS.filter(item => isAllowed(item.href, userProfile.clearance));
  const hasAccess = isAllowed(pathname, userProfile.clearance);
    ...
    <div className="min-h-screen bg-[#070814] text-white flex relative overflow-x-hidden">
      ...
      >
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/10 shrink-0">
          ...
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="text-text-muted hover:text-text-primary" aria-label={isSidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}>
        ...
        <div className="p-4 border-t border-white/10 shrink-0">
          <div className="flex items-center justify-between">
            {!isSidebarCollapsed && (
              <div className="flex items-center gap-3">
                <div className="h-9 w-9 rounded-full bg-accent-purple/20 border border-accent-purple/30 flex items-center justify-center font-display font-semibold text-accent-purple text-sm">
                  {userProfile.name.split(" ").map((w: string) => w[0]).join("")}
                </div>
            ...
            <button onClick={handleLogout} className="p-2 text-text-muted hover:text-red-400 rounded-lg transition" aria-label="Logout">
              <Power size={16} />
            </button>
          </div>
        </div>
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-surface-1/80 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-4 z-40">
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 text-text-primary rounded-lg border border-white/10 bg-white/5" aria-label="Toggle mobile menu">
              ...
              >
                <Card className="glass-card border border-red-500/25 bg-red-500/5 p-8 relative overflow-hidden shadow-glow-purple/20">
                  ...
                  <p className="text-xs text-text-muted leading-relaxed mt-2">
                    Clearance levels for AIESEC role <span className="text-accent-pink font-semibold font-mono">{userProfile.clearance}</span> do not cover access permissions for route <span className="text-text-primary font-mono">{pathname}</span>.
                  </p>
